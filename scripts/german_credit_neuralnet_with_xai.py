...
LIME_OUTPUT_DIR = OUTPUT_DIR / "lime"
SHAP_OUTPUT_DIR = OUTPUT_DIR / "shap"

for output_dir in (PR_CURVE_DIR, OUTPUT_DIR, LIME_OUTPUT_DIR, SHAP_OUTPUT_DIR):
...
f1_score_nn_test = round(
    f1_score(ytest_np, predicted_clf_nn_best_threshold), 3
)
...
x_train_test = torch.cat((xtrain, xtest))

# The following matrix indicates the sign of the aggregate contributions.
# matrix_shapley_values = pd.DataFrame(
#     np.vstack([vector for vector in shap_values]),
#     columns=feature_names
# )
# mean_abs_shapley_values = matrix_shapley_values.apply(
#     np.mean,
#     axis=0
# ).sort_values()
#
# plot = mean_abs_shapley_values.plot(
#     kind="bar",
#     title="Feature importance: SHAP"
# )
#
# plt.show()
# plt.savefig("../plots/german_credit/neuralnet_shap.png")


def model_wrapper(data):
    """
    Since lime cannot handle tensors, create a wrapper of the model
    that has numpy inputs and outputs.
    """
...
x_train_test = torch.cat((xtrain, xtest))
x_train_test_np = x_train_test.detach().numpy()

y_train_test_np = np.vstack((ytrain.detach().numpy(), ytest.detach().numpy())).reshape(
    [
        -1,
    ]
)

index_train_test = np.concatenate((df_train.index, df_test.index))

net.eval()

with torch.no_grad():
    inpts = torch.cat((train_ds.x_data, test_ds.x_data))
    p = net(inpts)

probabilities_train_test = (
    p.detach()
    .numpy()
    .reshape(
        [
...
model_wrapper.predict = model_wrapper
...
xpl_lime_neuralnet = SmartExplainer(model=model_wrapper, backend="lime")

xpl_lime_neuralnet.compile(
    x=pd.DataFrame(x_train_test_np, index=index_train_test, columns=feature_names),
    y_pred=pd.Series(probabilities_train_test, index=index_train_test),
    y_target=pd.Series(y_train_test_np, index=index_train_test),
)

app = xpl_lime_neuralnet.run_app(
    title_story="German credit: LIME for the neural network", port=8021
)
...
# background = x_train_test[
#     np.random.choice(x_train_test.shape[0], 1000, replace=False)
# ]

background = x_train_test[np.random.choice(x_train_test.shape[0], 100, replace=False)]

e = shap.DeepExplainer(net, background)

shap_values = e.shap_values(x_train_test)

matrix_shapley_values = pd.DataFrame(
    np.vstack([np.abs(vector) for vector in shap_values]), columns=feature_names
)


explainer = shap.KernelExplainer(model_wrapper, x_train_test.detach().numpy()[:50, :])

shap_values = explainer.shap_values(x_train_test.detach().numpy(), nsamples=500)
...
xpl_shap_neuralnet = SmartExplainer(model=model_wrapper, backend="shap")

# The next command returns an error. Probably because PyTorch and Shapash
# are not fully compatible and the compile method requires a contribution matrix.

matrix_shapley_values.index = index_train_test

xpl_shap_neuralnet.compile(
    x=pd.DataFrame(x_train_test_np, index=index_train_test, columns=feature_names),
    contributions=matrix_shapley_values,
    y_pred=pd.Series(probabilities_train_test, index=index_train_test),
    y_target=pd.Series(y_train_test_np, index=index_train_test),
)

app = xpl_shap_neuralnet.run_app(
    title_story="Explaining the neural network model for the German credit dataset using SHAP",
    port=8022,
)
...
xpl_lime_neuralnet.plot.features_importance()
xpl_shap_neuralnet.plot.features_importance()

xpl_lime_neuralnet.features_imp.to_csv(LIME_OUTPUT_DIR / "lime_neuralnet_all.csv")

xpl_shap_neuralnet.features_imp.to_csv(SHAP_OUTPUT_DIR / "shap_neuralnet_all.csv")

ql = xpl_lime_neuralnet.features_imp
qm = xpl_shap_neuralnet.features_imp

# spearmanr(ql.tail(10), qm.tail(10))
...
#     (
#         pd.DataFrame(xtrain.detach().numpy()),
#         pd.DataFrame(xtest.detach().numpy())
#     ),
#     axis=0
# )
#
# x_train_test.index = [*df_train.index, *df_test.index]
# feature_names = df_train.columns[1:]
#
#
## y_train_test = np.concatenate((ytrain.values, ytest.values))
# y_train_test = pd.concat(
#     (
#         pd.DataFrame(ytrain.detach().numpy()),
#         pd.DataFrame(ytest.detach().numpy())
#     ),
#     axis=0
# )
#
# y_train_test.index = x_train_test.index
#
# net.eval()
# probabilities_train_test = net(x_train_test)
#
## Plot the SHAP values
# shap.summary_plot(shap_values, x_train_test)
#
